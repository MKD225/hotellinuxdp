...
public class Panel_Reservation extends JPanel implements ActionListener,
		MouseListener, ReservationListener, PropertyChangeListener,
		ItemListener {
	...
	public Panel_Reservation() {
		...
		txtCheckIn.addPropertyChangeListener(this);
		txtCheckOut.addPropertyChangeListener(this);
		boxRoomType.addItemListener(this);
	...
	private JComponent createForm() {
		...
		txtArrivalTime.setForeground(new Color(252, 4, 4));
		...
		txtRoomRate.setForeground(new Color(252, 4, 4));
	...
	int checkInDay, checkInMonth, checkInYear, checkOutDay, checkOutMonth,
			checkOutYear;

	@Override
	public void propertyChange(PropertyChangeEvent e) {
		if (e.getSource() == txtCheckOut) {
			Calendar checkInCalendar = txtCheckIn.getCalendar();
			Calendar checkOutCalendar = txtCheckOut.getCalendar();
			...
			if (checkInDay < checkOutDay && checkInMonth == checkOutYear
					&& checkInYear == checkOutYear) {
			...
			if (checkInDay > checkOutDay && checkInMonth == checkOutMonth
				...
				JOptionPane.showMessageDialog(this,
						"ngày đặt phòng không thể nhỏ hơn ngày trả phòng");
			}
			if (checkInDay > checkOutDay && checkInMonth == checkOutMonth
					&& checkInYear > checkOutYear) {
				JOptionPane.showMessageDialog(this,
						"năm trả phòng không thể nhở hơn năm đặt phòng");
			...
			if (checkInDay > checkOutDay && checkInMonth > checkOutMonth
				...
				JOptionPane
						.showMessageDialog(this,
								"Ngày tháng đặt phòng không thể nhỏ hơn ngày trả phòng");
				txtArrivalTime.setText("");
			}
			if (checkInMonth < checkOutMonth && checkInYear == checkOutYear) {
				Long arrivalTL = (checkOutCalendar.getTime().getTime() - checkInCalendar
						.getTime().getTime()) / (24 * 3600 * 1000);
				txtArrivalTime.setText(arrivalTL.toString());
			}if(checkInYear>checkOutYear){
				JOptionPane.showMessageDialog(this,"năm trả phòng không thể nhở hơn năm đặt phòng");
				txtArrivalTime.setText("");
			}
		}
	}
	...
	public void itemStateChanged(ItemEvent e) {
		if (e.getSource() == boxRoomType) {
			int integer = boxRoomType.getSelectedIndex();
			try {
				RoomType rt = RoomTypeController.roomTypeController.all().get(integer);
				txtRoomRate.setText(rt.getRoomRate());
			} catch (SQLException e1) {
				e1.printStackTrace();
			}
		}
