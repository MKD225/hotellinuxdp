...
import hotel.linuxdp.java.controller.FoodsController;
...
public class Panel_Foods extends JPanel {
	...
	private JComponent jTable() {
		...
		jTable = new JTable();
		tableTitle.add("FoodID");
		tableTitle.add("FoodName");
		tableTitle.add("FoodPrice");
	...
	private void all() {
		try {
			for (Foods foods : FoodsController.foodsController.all()) {
				model.insertRow(0, foods.toArray());
			}
		} catch (Exception ex) {
			ex.printStackTrace();
		}
